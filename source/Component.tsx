...
async function VO0(A, B, Q, Z, G, Y = !0, I, W) {
	...
	let J = FO0({
		...
		X = fs();
	if (Y && X.existsSync(J))
		throw new Error(`Agent file already exists: ${J}`);
				...
				({
					...
					if (!q) {
						...
						return (
							...
							console.debug(
								`ZodCatchiled to parse agent from ${K}: ${E}`,
							),
...
var dj = memoize(async () => {
	...
	let W = B.enabledPlugins || {},
		J = Object.entries(W).filter(
			([K, z]) => K.startsWith('npm:') && z === !0,
		),
		X = await Promise.all(
			J.map(async ([K, z]) => {
				let H = K.slice(4);
	...
	let F = Object.keys(A.repositories).length,
		V = Object.entries(W).filter(
			([K, z]) => K.startsWith('npm:') && z === !0,
		).length;
	return (
		debugLog(
			`Found ${Q.length + Z.length} plugins (${Q.length} enabled, ${
				Z.length
			} disabled) from ${F} repositories and ${V} npm packages`,
		),
		{
			enabled: Q,
			disabled: Z,
...
function m15(A, B, Q, Z) {
	...
	try {
		...
		return (
			console.error(`ZodCatchiled to load agent from ${A}: ${Y}`), null
		);
	}
}
...
var T01 = memoize(async () => {
	...
	if (B.length > 0)
		console.debug(
			`Plugin loading errors: ${B.map(Z => Z.error).join(', ')}`,
		);
	for (let Z of A) {
		...
		try {
			...
			if ((Q.push(...G), G.length > 0))
				console.debug(
					`Loaded ${G.length} agents from plugin ${Z.name}`,
				);
		} catch (G) {
			console.error(
				`ZodCatchiled to load agents from plugin ${Z.name}: ${G}`,
			);
		}
	}
...
function eh1(A) {
	switch (A) {
		...
		case 'projectSettings':
			return join(
				getCurrentWorkingDirectory(),
				ConfigAgents.FOLDER_NAME,
				ConfigAgents.AGENTS_DIR,
			);
		case 'policySettings':
			return join(
				getSystemConfigPath(),
...
function AqB(A, B, Q) {
	...
	if (!A.agentType) Z.push('Agent type is required');
		...
		if (Y) Z.push(Y);
		let I = Q.find(
			W => W.agentType === A.agentType && W.source !== A.source,
		);
		if (I)
			Z.push(
				`Agent type "${A.agentType}" already exists in ${lA1(
					I.source,
				)}`,
			);
	}
	...
	if (!A.tools || !Array.isArray(A.tools)) Z.push('Tools must be an array');
		...
		if (Y.invalidTools.length > 0)
			Z.push(`Invalid tools: ${Y.invalidTools.join(', ')}`);
	...
	if (!A.systemPrompt) Z.push('System prompt is required');
		...
		Z.push('System prompt is too short (minimum 20 characters)');
 ...
 */
function AgentConfirmationStep({
	tools: A,
	existingAgents: B,
	onSave: Q,
	onSaveAndEdit: Z,
		...
		createElement(
			...
			createElement(
				...
				createElement(
					M,
					null,
					W.whenToUse.length > 240
						? W.whenToUse.slice(0, 240) + '…'
						: W.whenToUse,
				),
			),
			createElement(
			...
			createElement(
				...
				createElement(
					M,
					null,
					W.systemPrompt.length > 240
						? W.systemPrompt.slice(0, 240) + '…'
						: W.systemPrompt,
				),
			),
			J.warnings.length > 0 &&
