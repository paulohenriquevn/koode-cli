...
class ApplicationStateManager {
	...
	constructor() {
		status.debug(
			...
			'Initializing ApplicationStateManager',
		);
	...
	private detectAndUpdateWorkspace(): void {
		...
		const detectedLanguages = this.detectLanguages(
			projectRoot || currentDir,
		);
		...
		status.info(
			DevToolCategory.SYSTEM,
			`Workspace detected: ${projectType} project in ${projectName}`,
		);
	}
		...
		if (existsSync(packageJsonPath)) {
			try {
				...
				if (
					...
					packageJson.devDependencies?.['nest']
				)
					return 'nestjs';
				if (
	...
	private updateProjectState(projectDir: string): void {
		...
		projectState.mainFiles = this.findFilesByPattern(projectDir, [
			'index.*',
			'main.*',
			'app.*',
		]);
		projectState.testFiles = this.findFilesByPattern(projectDir, [
			'*.test.*',
			'*.spec.*',
			'__tests__/*',
		]);
		projectState.configFiles = this.findFilesByPattern(projectDir, [
			'*.config.*',
			'.*rc.*',
			'*.yaml',
			'*.yml',
	...
	public updateState(updates: Partial<ApplicationState>): void {
		...
		status.debug(DevToolCategory.SYSTEM, 'Application state updated', {
	...
	public changeWorkspace(newPath: string): boolean {
		try {
			...
			if (
				...
				status.error(
					DevToolCategory.SYSTEM,
					`Invalid workspace path: ${resolvedPath}`,
				);
				return false;
			}
			...
			status.success(
				DevToolCategory.SYSTEM,
				`Workspace changed to: ${resolvedPath}`,
			);
			return true;
		} catch (error) {
			status.error(DevToolCategory.SYSTEM, 'Failed to change workspace', {
				error,
				newPath,
			});
			return false;
	...
	public enableTool(toolName: string): boolean {
		const tool = this.state.tools.availableTools.find(
			t => t.name === toolName,
		);
		if (!tool) {
	...
	public recordToolExecution(execution: ToolExecution): void {
		this.updateState({
			tools: {
				...
				toolExecutionHistory: [
					...this.state.tools.toolExecutionHistory,
					execution,
				],
			},
		});
	...
	public isConfigured(): boolean {
		return (
			this.state.configuration.isConfigured &&
			!!this.state.configuration.apiKey
		);
	}
	...
	public printStatus(): void {
		...
		console.log('\n📊 Application Status:');
		console.log(
			`📁 Workspace: ${workspace.projectName || 'Unknown'} (${
		...
		console.log(
			`🛠️  Languages: ${
				workspace.detectedLanguages.join(', ') || 'None detected'
			}`,
		);
		console.log(
			`⚙️  Configuration: ${
				configuration.isConfigured ? '✅ Loaded' : '❌ Not configured'
			}`,
		);
		console.log(`🤖 Model: ${configuration.currentModel}`);
		console.log(
			`🔧 Enabled Tools: ${tools.enabledTools.length}/${tools.availableTools.length}`,
		);
