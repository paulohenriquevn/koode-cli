...
class DevToolManager {
  ...
  constructor() {
    ...
    this.state = {
      isDebugMode: process.env.DEBUG === 'true' || process.argv.includes('--debug'),
      isVerboseMode: process.env.VERBOSE === 'true' || process.argv.includes('--verbose'),
  ...
  private shouldLog(category: DevToolCategory, message: string): boolean {
    ...
    if (this.state.debugFilter) {
      return this.state.debugFilter.some(filter => 
        category.includes(filter) || message.toLowerCase().includes(filter)
      );
    }
  ...
  private logWithStyle(level: StatusLevel, category: DevToolCategory, message: string, data?: any): void {
    ...
    if (level === StatusLevel.ERROR) {
      ...
      if (data) this.writeToStderr(`${dim}Data: ${JSON.stringify(data, null, 2)}${reset}`);
    } else {
      this.writeToStdout(formattedMessage);
      if (data && this.state.isVerboseMode) {
        this.writeToStdout(`${dim}Data: ${JSON.stringify(data, null, 2)}${reset}`);
      }
    }
  }
  ...
  public success(category: DevToolCategory, message: string, data?: any): void {
  ...
  public startOperation(operationName: string): void {
    ...
    this.debug(DevToolCategory.SYSTEM, `Starting operation: ${operationName}`);
  }

  public endOperation(success: boolean = true, message?: string): void {
    if (!this.state.currentOperation || !this.state.operationStartTime) return;
    ...
    if (success) {
      this.success(DevToolCategory.SYSTEM, 
        `Completed operation: ${operationName} (${duration}ms)${message ? ` - ${message}` : ''}`
      );
    } else {
      this.error(DevToolCategory.SYSTEM,
        `Failed operation: ${operationName} (${duration}ms)${message ? ` - ${message}` : ''}`
      );
    }
  }
  ...
  public printSummary(): void {
    ...
    this.info(DevToolCategory.SYSTEM, `🎯 Operations: ${stats.totalOperations}`);
    this.info(DevToolCategory.SYSTEM, `✅ Successful: ${stats.successfulOperations}`);
    this.info(DevToolCategory.SYSTEM, `❌ Failed: ${stats.failedOperations}`);
    this.info(DevToolCategory.SYSTEM, `🚀 Total Duration: ${stats.totalDuration}ms`);
    
    if (stats.totalOperations > 0) {
      const avgDuration = Math.round(stats.totalDuration / stats.totalOperations);
      const successRate = Math.round((stats.successfulOperations / stats.totalOperations) * 100);
      this.info(DevToolCategory.SYSTEM, `📈 Avg Duration: ${avgDuration}ms`);
      this.info(DevToolCategory.SYSTEM, `🎉 Success Rate: ${successRate}%`);
    }
  }
}
...
export const status = {
  ...
  enableCategory: (category: DevToolCategory) => statusManager.enableCategory(category),
  disableCategory: (category: DevToolCategory) => statusManager.disableCategory(category),
