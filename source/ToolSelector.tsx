...
function jG5(A) {
	...
	return (
		...
		Array.from(B.entries())
			...
			.sort((Q, Z) =>
				Q.serveZodArrayame.localeCompare(Z.serveZodArrayame),
			)
	);
}
...
function VqA(A) {
	return (
		A === 12288 || (A >= 65281 && A <= 65376) || (A >= 65504 && A <= 65510)
	);
}

function d$9(A) {
	if (!Number.isSafeInteger(A))
		throw new TypeError(`Expected a code point, got \`${typeof A}\`.`);
}
...
var aU1 = A => {
	let B = {
		...
		parentNode: void 0,
		yogaNode:
			A === 'ink-virtual-text' ? void 0 : yogaLayoutEngine.Node.create(),
	};
...
var FuseSearch9 = (A, B) => {
		if ('position' in B)
			A.setPositionType(B.position === 'absolute' ? bU1 : vU1);
	},
	Iw9 = (A, B) => {
	...
	Jw9 = (A, B) => {
		...
		if ('flexShrink' in B)
			A.setFlexShrink(
				typeof B.flexShrink === 'number' ? B.flexShrink : 1,
			);
		if ('flexWrap' in B) {
		...
		if ('alignItems' in B) {
			if (B.alignItems === 'stretch' || !B.alignItems)
				A.setAlignItems(wU1);
		...
		if ('justifyContent' in B) {
			if (B.justifyContent === 'flex-start' || !B.justifyContent)
				A.setJustifyContent(jU1);
	...
	Xw9 = (A, B) => {
		if ('width' in B)
			if (typeof B.width === 'number') A.setWidth(B.width);
				...
				A.setWidthPercent(Number.parseInt(B.width, 10));
			else A.setWidthAuto();
		if ('height' in B)
			if (typeof B.height === 'number') A.setHeight(B.height);
			else if (typeof B.height === 'string')
				A.setHeightPercent(Number.parseInt(B.height, 10));
			else A.setHeightAuto();
		if ('minWidth' in B)
			if (typeof B.minWidth === 'string')
				A.setMinWidthPercent(Number.parseInt(B.minWidth, 10));
			else A.setMinWidth(B.minWidth ?? 0);
		if ('minHeight' in B)
			if (typeof B.minHeight === 'string')
				A.setMinHeightPercent(Number.parseInt(B.minHeight, 10));
...
var bEA = (A, B) => {
	...
	for (let G of Object.keys(A))
		if (B ? !Object.hasOwn(B, G) : !0) (Q[G] = void 0), (Z = !0);
	if (B) {
		for (let G of Object.keys(B))
			if (B[G] !== A[G]) (Q[G] = B[G]), (Z = !0);
	}
	return Z ? Q : void 0;
};
...
var i10 = (A, B, Q) => {
	...
	if (Z >= 0) {
		if ((A.childNodes.splice(Z, 0, B), B.yogaNode))
			A.yogaNode?.insertChild(B.yogaNode, Z);
		return;
	}
...
var ug = react_reconciler.default({
	...
	resetAfterCommit(A) {
		...
		if (A.isStaticDirty) {
			if (
				((A.isStaticDirty = !1),
				typeof A.onImmediateRender === 'function')
			)
				A.onImmediateRender();
			return;
		}
	...
	let G = useMemo(() => gwB(A), [A]),
		...
		q = () => {
			let S = G.map(o => o.name),
				u =
					K.length === S.length && S.every(o => K.includes(o))
						? ['*']
						: K;
			Q(u);
		},
		E = useMemo(() => {
			...
			return (
				G.forEach(u => {
					...
					else if (S.READ_ONLY.toolNames.has(u.name))
						c.readOnly.push(u);
					else if (S.EDIT.toolNames.has(u.name)) c.edit.push(u);
					else if (S.EXECUTION.toolNames.has(u.name))
						c.execution.push(u);
	...
	if (F) {
		...
		G.forEach(S => {
			...
			if (S.name.startsWith('mcp__')) {
				let u = hk(S.name);
				c = u
					? `${u.tooShellErrorame} (${u.serveZodArrayame})`
					: S.name;
			}
			O.push({
				id: `tool-${S.name}`,
	...
	return (
		...
		(
			<Box flexDirection="column" marginTop={1}>
				<Text color={J === 0 ? 'suggestion' : undefined} bold={J === 0}>
					{J === 0 ? `${e0.pointer} ` : '  '}[ Continue ]
				</Text>
				<Text dimColor={true}>{'─'.repeat(40)}</Text>
				{O.slice(1).map((S, c) => {
					let u = c + 1 === J,
						o = S.isToggle,
						m = S.isHeader;
					return (
						<Fragment key={S.id}>
							{o && <Text dimColor={true}>{'─'.repeat(40)}</Text>}
							{m && c > 0 && <Box marginTop={1} />}
							<Text
								color={
									m ? undefined : u ? 'suggestion' : undefined
								}
								dimColor={m}
								bold={o && u}
							>
				...
				<Box marginTop={1} flexDirection="column">
					<Text dimColor={true}>
						{H
							? 'All tools selected'
							: `${z.size} of ${G.length} tools selected`}
					</Text>
				</Box>
			</Box>
		)
